...
public:
	...
	VertexShader(ID3D11Device& device, ID3D11DeviceContext& context, std::string fileName) : Shader(device, context, fileName) {
		ASSERT(device.CreateVertexShader(code.data, code.dataLength, nullptr, &shader));
	}

public:
	void SetActive() const NOEXCEPT_RELEASE override {
...
public:
	...
	PixelShader(ID3D11Device& device, ID3D11DeviceContext& context, std::string fileName) : Shader(device, context, fileName) {
		ASSERT(device.CreatePixelShader(code.data, code.dataLength, nullptr, &shader));
	}
